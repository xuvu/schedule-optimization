    ...
    for d in all_days:
        for s in all_shifts:
            for t1 in all_types:
                for t2 in all_types:
                    if t1 != t2:
                        model.Add(sum(shifts[(n, d, s)] for n in all_nurses if type_of_nurse(n) == t1) +
                                  sum(shifts[(n, d, s)] for n in all_nurses if type_of_nurse(n) == t2) >= 1)

    # Prioritize (1,2) to be same person
    for d in all_days:
        for n in all_nurses:
            model.Add(shifts[(n, d, 1)] <= shifts[(n, d, 2)])
