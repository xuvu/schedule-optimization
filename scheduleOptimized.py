...
def main(m, decrement_, current_state):
    ...
    for d in all_days:
        for n in all_nurses:
            ...
            if (d + day_interval) < len(all_days):  # 1 + (4-1) < 31
                model.Add(sum(shifts[(n, d + i, s, k)] for i in range(day_interval + 1) for s in all_shifts for k in
    ...
    for n in all_nurses:
        ...
        max_slack = 0
        min_slack = -1
        total_max_child_hospital = (((total_holidays * (num_shifts - 0)) + (total_working_days * (num_shifts - len(forbidden_shifts)))) * (num_of_nurse_per_shift - 2))
    ...
    if status == cp_model.OPTIMAL:
        ...
        for d in all_days:
            ...
            if d not in all_weekends:
                sheet['A' + str(current_shift_cell[0])] = str(d)
                sheet['A' + str(current_shift_cell_[0])] = str(d)
            else:
                ...
                sheet['A' + str(current_shift_cell[0] + 1)].fill = redFill
                sheet['A' + str(current_shift_cell_[0] + 1)].fill = redFill

                sheet['A' + str(current_shift_cell[0] + 2)].fill = redFill
                sheet['A' + str(current_shift_cell_[0] + 2)].fill = redFill

            for s in all_shifts:
                if s != 0 and d in all_working_days:  # skip morning shift
                    sheet['B' + str(current_shift_cell[s])] = shift_cell_text[s]
                    sheet['B' + str(current_shift_cell_[s])] = shift_cell_text[s]
                elif d in all_weekends:
                    sheet['B' + str(current_shift_cell[s])] = shift_cell_text[s]
                    sheet['B' + str(current_shift_cell_[s])] = shift_cell_text[s]

                    sheet['B' + str(current_shift_cell[s])].fill = redFill
                    sheet['B' + str(current_shift_cell_[s])].fill = redFill
                ...
                for pair_ in pair_val:
                    ...
                    if type_of_nurse(pair_[1]) == 0:
                        sheet[shift_column[column_count] + str(current_shift_cell[s])].fill = blueFill
                        sheet[shift_column[column_count] + str(current_shift_cell[s])] = map_name_person(pair_[1])
        ...
        for c in current_cell_num:
            for j in range(len(field_summary)):
                ...
                for k in range(len(field_summary[j])):
                    ...
                    sheet[field_summary[j][k] + str(c - 1 + fixable_section_cell_num)] = shift_cell_text[
                        shift_cell_text_pos]
                    sheet[field_summary[j][k] + str(c - 1 + fixable_section_cell_num)] = shift_cell_text[
                        shift_cell_text_pos]
                    sheet[field_summary[j][k] + str(c - 1 + fixable_section_cell_num)] = shift_cell_text[
        ...
        for n in all_nurses:
            ...
            # Summary name
            # [['H', 'I', 'J', 'K'], ['N', 'O', 'P', 'Q'], ['T', 'U', 'V', 'W']]
            if type_of_nurse(n) == 0:
                sheet['G' + str(summary_cell_num + 1)].fill = blueFill
                sheet['G' + str(summary_cell_num + 1)] = map_name_person(n)

                sheet['G' + str(summary_cell_num + 1 + fixable_section_cell_num)].fill = current_color
                sheet['G' + str(summary_cell_num + 1 + fixable_section_cell_num)] = map_name_person(n)
            else:
                sheet['G' + str(summary_cell_num + 1)].fill = greyFill
                sheet['G' + str(summary_cell_num + 1)] = map_name_person(n)
            ...
            for s in range(len(count_person_cell[0])):
                for k in range(len(count_person_cell[0][s])):
                    ...
                    if len(count_person_cell[0][s][k]) > 0:
                        ...
                        if k != 0:
                            count_cell_working_main[count_cell_name[s]] += '+COUNTIF(' + start_end[0] + ':' + start_end[
                                1] + ',' + '"' + map_name_person(n) + '")'  # add start and end cell for counting
                            count_cell_working_child[count_cell_name[s]] += '+COUNTIF(' + temp[
                                0] + ',' + '"' + map_name_person(n) + '")'
                        else:
                            count_cell_working_main[count_cell_name[s]] += '=COUNTIF(' + start_end[0] + ':' + start_end[
                                1] + ',' + '"' + map_name_person(n) + '")'  # add start and end cell for counting
            ...
            for s in range(len(count_person_cell[1])):
                for k in range(len(count_person_cell[1][s])):
                    ...
                    if len(count_person_cell[1][s][k]) > 0:
                        ...
                        if k != 0:
                            count_cell_holiday_main[count_cell_name[s]] += '+COUNTIF(' + start_end[0] + ':' + start_end[
                                1] + ',' + '"' + map_name_person(n) + '")'  # add start and end cell for counting
                            count_cell_holiday_child[count_cell_name[s]] += '+COUNTIF(' + temp[
                                0] + ',' + '"' + map_name_person(n) + '")'
                        else:
                            count_cell_holiday_main[count_cell_name[s]] += '=COUNTIF(' + start_end[0] + ':' + start_end[
                                1] + ',' + '"' + map_name_person(n) + '")'  # add start and end cell for counting
                ...
                for j in range(len(field_summary) - 1):
                    ...
                    for k in range(len(field_summary[j]) - 1):
                        sheet[field_summary[j][k] + str(current_cell_num[0] + fixable_section_cell_num)] = \
                            main_count[j][k]
                        sheet[field_summary[j][k] + str(
                            current_cell_num[0] + fixable_section_cell_num)].fill = current_color
                    ...
                    sheet[field_summary[j][3] + str(current_cell_num[0] + fixable_section_cell_num)] = '=SUM(' + \
                                                                                                       ...
                                                                                                       sum_cell[
                                                                                                           len(sum_cell) - 1] + ')'
                    sheet[
                        field_summary[j][3] + str(current_cell_num[0] + fixable_section_cell_num)].fill = current_color
                ...
                for j in range(len(field_summary) - 1):
                    ...
                    for k in range(len(field_summary[j]) - 1):
                        sheet[field_summary[j][k] + str(current_cell_num[1] + fixable_section_cell_num)] = \
                            child_count[j][k]
                        sheet[field_summary[j][k] + str(
                            current_cell_num[1] + fixable_section_cell_num)].fill = current_color
                    ...
                    sheet[field_summary[j][3] + str(current_cell_num[1] + fixable_section_cell_num)] = '=SUM(' + \
                                                                                                       ...
                                                                                                       sum_cell[
                                                                                                           len(sum_cell) - 1] + ')'
                    sheet[
                        field_summary[j][3] + str(current_cell_num[1] + fixable_section_cell_num)].fill = current_color
            # Final Summary
            ...
            sum_working_day_cell = field_summary[0][len(field_summary[0]) - 2] + str(
                summary_cell_num + 1 + fixable_section_cell_num)
            working_day_main = str(field_summary[0][len(field_summary[0]) - 1]) + str(
                current_cell_num[0] + fixable_section_cell_num)
            working_day_child = str(field_summary[0][len(field_summary[0]) - 1]) + str(
                current_cell_num[1] + fixable_section_cell_num)
            ...
            sum_holiday_cell = field_summary[0][len(field_summary[0]) - 3] + str(
                summary_cell_num + 1 + fixable_section_cell_num)
            holiday_main = str(field_summary[1][len(field_summary[0]) - 1]) + str(
                current_cell_num[0] + fixable_section_cell_num)
            holiday_child = str(field_summary[1][len(field_summary[0]) - 1]) + str(
                current_cell_num[1] + fixable_section_cell_num)
            ...
            sum_all_cell = field_summary[0][len(field_summary[0]) - 1] + str(
                summary_cell_num + 1 + fixable_section_cell_num)
...
if __name__ == '__main__':
    all_month = range(3, 4)
    decrement = {'diff_type': 0, 'con_shift': -6, 'div_factor': 4}
    increment = {'diff_type': 0, 'con_shift': -6}
    state_tracking = {'max-consecutive-shifts': [False, False], 'max-different-type-shifts': [False, False]}
    for m in all_month:
        while not(state_tracking['max-consecutive-shifts'] == [True, True] and state_tracking['max-different-type-shifts'] == [True, True]):

            # decrement state max-consecutive-shifts
            if state_tracking['max-consecutive-shifts'] == [False, False] and state_tracking['max-different-type-shifts'] == [False, False]:
                state_tracking = main(m, decrement, state_tracking)
                decrement['con_shift'] += 6 * decrement['div_factor']

            # increment state max-consecutive-shifts
            elif state_tracking['max-consecutive-shifts'] == [True, False] and state_tracking['max-different-type-shifts'] == [False, False]:
                state_tracking = main(m, decrement, state_tracking)
                decrement['con_shift'] -= 2

            # decrement state max-different-type-shifts
            elif state_tracking['max-consecutive-shifts'] == [True, True] and state_tracking['max-different-type-shifts'] == [False, False]:
                state_tracking = main(m, decrement, state_tracking)
                decrement['diff_type'] += 6 * decrement['div_factor']

            # decrement state max-different-type-shifts
            elif state_tracking['max-consecutive-shifts'] == [True, True] and state_tracking['max-different-type-shifts'] == [True, False]:
                state_tracking = main(m, decrement, state_tracking)
